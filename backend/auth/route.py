...
@auth_router.post(path="/register", response_model=UserPublic, status_code=status.HTTP_201_CREATED)
def register(user: UserCreate,background_tasks: BackgroundTasks,  session: Session = Depends(dependency=get_session)) -> User:
...
@auth_router.post(path="/login", response_model=AccessToken, status_code=status.HTTP_200_OK)
def login(form_data: LoginRequest, session: Session = Depends(dependency=get_session)) -> AccessToken:
    """ login the user (only verified user can login)"""
    user: User | None = authenticate_user(session=session, username=form_data.email, password=form_data.password)
    if not user:
        raise HTTPException(
